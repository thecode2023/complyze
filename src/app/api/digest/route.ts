...
// POST: Generates weekly digest for all onboarded users (protected by CRON_SECRET)
export async function POST(request: NextRequest) {
  ...
  // Get all regulatory updates from the last 7 days
  ...
  for (const user of users ?? []) {
    try {
      // Filter updates to those matching user's jurisdictions
      ...
      if (relevantUpdates.length === 0) {
        // Store a "no changes" digest
        await supabase.from("weekly_digests").insert({
          user_id: user.id,
          digest_content: {
            briefing: "No regulatory changes affecting your profile this week.",
            updates_count: 0,
...
- Only reference regulations and updates listed above. Do not invent or cite any regulation not in the list.`;
      ...
      // Log but don't fail the entire run for one user
...
// Also support GET for Vercel Cron (Vercel Cron sends GET requests to trigger)
// We differentiate: if CRON_SECRET is present, treat as POST (generate digests)
// Otherwise, return the user's latest digest
