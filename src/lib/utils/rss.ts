import Parser from "rss-parser";
import type { SupabaseClient } from "@supabase/supabase-js";

const parser = new Parser({
  timeout: 15000,
  headers: {
    "User-Agent": "Complyze-Regulatory-Intelligence/1.0",
    "Accept": "application/rss+xml, application/atom+xml, application/xml, text/xml",
  },
});

export interface RSSFeedItem {
  title: string;
  link: string;
  contentSnippet: string;
  pubDate: string;
  source: string;
}

export interface RSSSource {
  name: string;
  url: string;
  category: string;
}

export interface FeedResult {
  items: RSSFeedItem[];
  errors: { source: string; error: string }[];
  sourceCount: number;
  fromDatabase: boolean;
}

const FALLBACK_FEEDS: RSSSource[] = [
  { name: "Federal Register — AI and Technology", url: "https://www.federalregister.gov/api/v1/documents.rss?conditions%5Btopic_ids%5D%5B%5D=artificial-intelligence", category: "US Federal" },
  { name: "EUR-Lex — Parliament and Council Legislation", url: "https://eur-lex.europa.eu/EN/display-feed.rss?rssId=162", category: "EU" },
  { name: "EUR-Lex — Commission Proposals", url: "https://eur-lex.europa.eu/EN/display-feed.rss?rssId=161", category: "EU" },
];

/**
 * Fetch feed sources from the database. Falls back to hardcoded list if DB unavailable.
 */
export async function getFeedSources(supabase: SupabaseClient): Promise<{ sources: RSSSource[]; fromDatabase: boolean }> {
  try {
    const { data, error } = await supabase
      .from("feed_sources")
      .select("name, url, category")
      .eq("enabled", true);

    if (error) {
      console.error(`[RSS] feed_sources query error: ${error.message} (code: ${error.code})`);
      console.warn("[RSS] Falling back to hardcoded feeds (3)");
      return { sources: FALLBACK_FEEDS, fromDatabase: false };
    }

    if (!data || data.length === 0) {
      console.warn("[RSS] feed_sources table returned 0 enabled rows — falling back to hardcoded feeds (3)");
      return { sources: FALLBACK_FEEDS, fromDatabase: false };
    }

    console.log(`[RSS] Loaded ${data.length} feed sources from database: ${data.map((d) => d.name).join(", ")}`);
    return { sources: data, fromDatabase: true };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error(`[RSS] Exception loading feed_sources: ${msg}`);
    console.warn("[RSS] Falling back to hardcoded feeds (3)");
    return { sources: FALLBACK_FEEDS, fromDatabase: false };
  }
}

export async function fetchRSSFeed(source: RSSSource): Promise<RSSFeedItem[]> {
  try {
    const feed = await parser.parseURL(source.url);
    const items = (feed.items || []).slice(0, 10).map((item) => ({
      title: String(item.title || "Untitled"),
      link: String(item.link || ""),
      contentSnippet: String(item.contentSnippet || item.content || ""),
      pubDate: String(item.pubDate || new Date().toISOString()),
      source: source.name,
    }));
    console.log(`[RSS] ✓ "${source.name}" — ${items.length} items`);
    return items;
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error(`[RSS] ✗ "${source.name}" — ${msg}`);
    return [];
  }
}

/**
 * Update feed source status in the database after fetch attempt.
 */
export async function updateFeedStatus(
  supabase: SupabaseClient,
  url: string,
  success: boolean,
  errorMsg?: string
) {
  try {
    if (success) {
      await supabase
        .from("feed_sources")
        .update({
          last_fetched_at: new Date().toISOString(),
          last_error: null,
          consecutive_failures: 0,
        })
        .eq("url", url);
    } else {
      const { data } = await supabase
        .from("feed_sources")
        .select("consecutive_failures")
        .eq("url", url)
        .single();

      const failures = (data?.consecutive_failures ?? 0) + 1;
      await supabase
        .from("feed_sources")
        .update({
          last_error: errorMsg ?? "Unknown error",
          consecutive_failures: failures,
          enabled: failures < 5,
        })
        .eq("url", url);

      if (failures >= 5) {
        console.warn(`[RSS] Auto-disabled feed "${url}" after ${failures} consecutive failures`);
      }
    }
  } catch {
    // Don't let status update failures break the pipeline
  }
}

export async function fetchAllRSSFeeds(supabase: SupabaseClient): Promise<FeedResult> {
  const { sources, fromDatabase } = await getFeedSources(supabase);
  const errors: { source: string; error: string }[] = [];

  console.log(`[RSS] Starting fetch of ${sources.length} feeds (source: ${fromDatabase ? "database" : "hardcoded fallback"})`);

  const results = await Promise.allSettled(
    sources.map(async (source) => {
      try {
        const items = await fetchRSSFeed(source);
        if (fromDatabase) {
          await updateFeedStatus(supabase, source.url, items.length > 0, items.length === 0 ? "No items returned" : undefined);
        }
        if (items.length === 0) {
          errors.push({ source: source.name, error: "No items returned" });
        }
        return items;
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        errors.push({ source: source.name, error: msg });
        if (fromDatabase) {
          await updateFeedStatus(supabase, source.url, false, msg);
        }
        return [];
      }
    })
  );

  const items = results.flatMap((result) =>
    result.status === "fulfilled" ? result.value : []
  );

  const successCount = results.filter(
    (r) => r.status === "fulfilled" && r.value.length > 0
  ).length;

  console.log(
    `[RSS] Complete: ${items.length} items from ${successCount}/${sources.length} feeds` +
    (errors.length > 0 ? ` | ${errors.length} failed: ${errors.map((e) => e.source).join(", ")}` : "")
  );

  return { items, errors, sourceCount: sources.length, fromDatabase };
}
